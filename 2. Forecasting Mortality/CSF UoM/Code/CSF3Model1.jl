...
@everywhere begin
    ...
    @model function model1(d, loge, h)
        ...
        if any([size(d[i]) != size(loge[i]) for i in 1:8]) || any([size(d[1]) != size(d[i]) for i in 2:8])
            ...
            # Priors for b
            b1 ~ Dirichlet(ones(n_ages))
            b2 ~ Dirichlet(ones(n_ages))
            b3 ~ Dirichlet(ones(n_ages))
            b4 ~ Dirichlet(ones(n_ages))            
            b5 ~ Dirichlet(ones(n_ages))
            b6 ~ Dirichlet(ones(n_ages))
            b7 ~ Dirichlet(ones(n_ages))
            b8 ~ Dirichlet(ones(n_ages))
...
# Specification Model 1 (Univariate)
modelesp1 = model1(d, loge, 15)
...
@time perform_sampling_distributed(modelesp1, 10)

# Perform sampling
# Here, the @time macro is used for a simple timing of the function execution
@time chain_model1 = perform_sampling_distributed(modelesp1, 1000)

# Export the results
serialize("$(Output)chain_model1.jls", chain_model1)