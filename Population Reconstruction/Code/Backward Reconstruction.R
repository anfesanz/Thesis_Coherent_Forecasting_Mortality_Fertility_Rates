...
rec <- reconstruction_ext

#Defining the variables in the interation
baseline_year = 2018
...
while (baseline_year > 1998) {
  ...
  left_join(sredu %>% filter(year == baseline_year))
  ...
  rec <- rbind(rec, reconstruction_ext)

}

saveRDS(rec, "Data Created/rec.rds")
